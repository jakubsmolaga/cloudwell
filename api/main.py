...
def get_measurements():
    # Get last temperature
    query = 'from(bucket: "cloudwell") |> range(start: -24h) |> filter(fn: (r) => r._measurement == "temperature")'
    tables = query_api.query(query)
    # Convert data to JSON
    temperatures = []
    for table in tables:
    ...
    # Get last humidity
    query = 'from(bucket: "cloudwell") |> range(start: -24h) |> filter(fn: (r) => r._measurement == "humidity")'
    tables = query_api.query(query)
    # Convert data to JSON
    humidities = []
    for table in tables:
